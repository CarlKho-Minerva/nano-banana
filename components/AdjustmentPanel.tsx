...
const AdjustmentPanel: React.FC<AdjustmentPanelProps> = ({ onApplyAdjustment, isLoading }) => {
  ...
  return (
    <div className="w-full border border-white/20 rounded-lg p-4 flex flex-col gap-4 animate-fade-in">
      <h3 className="text-lg font-semibold text-center text-white">Apply a Professional Adjustment</h3>
      ...
      <input
        ...
        className="flex-grow bg-black border border-white/20 text-white rounded-lg p-4 focus:ring-2 focus:ring-white focus:outline-none transition w-full disabled:cursor-not-allowed disabled:opacity-60 text-base"
        disabled={isLoading}
      />
      ...
      {activePrompt && (
        <div className="animate-fade-in flex flex-col gap-4 pt-2">
            <button
                onClick={handleApply}
                className="w-full bg-white text-black font-bold py-4 px-6 rounded-lg transition-all duration-300 ease-in-out hover:bg-white/90 active:scale-95 text-base disabled:bg-white/50 disabled:cursor-not-allowed disabled:transform-none"
